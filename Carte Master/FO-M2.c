...
//---------------- Variables Globales -----------------------------------------
//-----------------------------------------------------------------------------
INFORMATIONS_SERIALIZER Reponse_Serial ;			//variable � renvoyer � FO-M6
INFORMATIONS_SERIALIZER empty = {0};
char command_to_transmit[40] = {0};						//variable contenant la commande a transmettre � Serializer
...
INFORMATIONS_SERIALIZER FO_M2(COMMANDES_SERIALIZER x){	
		...
		create_command(x);									//on cr�e la commande qui correspond � celle demand�e en parametre
		vider_chaine(reponse);							//on s'assure d'avoir une variable reponse vide pour acceuillir la nouvelle reponse � la commande envoy�e
		send_command(command_to_transmit);	//on envoie la commande � Serializer
		wait_for_answer();									//on attend la reponse de Serializer si n�cessaire
		create_infos() ;										//on transforme la reponse re�ue dans le bon format
	
		return Reponse_Serial ;								//on retourne la variable de type INFORMATIONS_SERIALIZER � FO-M6
}
//----------------------------------------------------------------------------
//----------------------------------------------------------------------------
...
void Uart1_int(void) interrupt 20 {
		if ( SCON1 & (1<<0) ){				//on regarde s'il s'agit d'une reception
				recup_infos();						//on appelle la fonction pour recuperer le caractere re�u
...
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
//--------- Cr�ation d'une commande au format Serializer --------------------------
...
//----------------------------------------------------------------------------------------------				
//---------- Fonction d'envoi de la commande � la carte Serializer -----------------------------
void send_command(char *x) {
		char i = 0 ;
		while (*(x+i) != '\0') {			//on parcourt la chaine tant que le caract�re re�u n'est pas le caractere de fin \0 
...
void wait_for_answer(void){				//on verifie maintenant si on attend une reponse ou non pour la commande envoy�e
...
//-------------------------------------------------------------------------------------------------
//----------- Fonction r�cup�ration informations Serializer ---------------------------------------------				
