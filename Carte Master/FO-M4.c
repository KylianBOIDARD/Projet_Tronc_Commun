...
//
//-----------------------------------------------------------------------------
//-------- Utilisation du t�l�m�tre Ultrasons ---------------------------------
#include "FO-M4.h"

//------------------------------------------------------------------------------
//---------------- Initialisation des variables globales -----------------------
//------------------------------------------------------------------------------
sbit  TRIGGER_AV = P0^5 ; //T�l�m�tre Avant Commande
sbit	ECHO_AV = P3^6 ;		//R�ponse AV
sbit  TRIGGER_AR = P0^6 ; //T�l�m�tre Arri�re
sbit	ECHO_AR = P3^7 ;		//R�ponse AR
...
void Init_FO_M4(void){
	...
	P3IF |= (1<<2) ;	//d�clenche sur front montant
	EIE2 |= (1<<5) ;  //enable intext7
	P3IF |= (1<<3) ;	//d�clenche sur front montant
	TRIGGER_AV = 0 ;	//initialise les signaux
	TRIGGER_AR = 0 ;	//de commande des t�l�m�tres 
...
void MES_Dist_AV (void) {
	...
	attente_mesure = 0 ;					//on est rentr� dans l'int donc la mesure a commenc�e
	while (ECHO_AV != 0); 					//tant qu'on est encore dans la mesure
	T4CON = 0x00 ; 								//on coupe le T4
	temps_mesure = (TH4<<8) + TL4 ;  		//on recupere le temps compt�
	if( overflow == 1 || temps_mesure > 0xB4D8 ){	//si t4 overflow, donc que temp > 35ms ou que temps > 25ms
		//la deuxieme condition est une s�curit�, normalement c'est 25ms max ou alors 38ms
			distance = 0 ;
	}
	else{
...
void EXT6_int(void) interrupt 18{
		...
		attente_mesure = 1 ;	//on indique qu'on a commenc� a compter	
}
...
void Timer4_int(void) interrupt 16{
		T4CON &= ~(1<<7) ;	//remise � z�ro du flag
		overflow = 1 ;
}

//------------------------------------------------------------------------------
//-------- Fonction G�n�ratrice de Signaux de Commande pour t�l�metre ----------
//------------------------------------------------------------------------------
...
unsigned int convert (int x) {
	unsigned int dist = 0;
	//le timer s'incr�mente toutes les 0.54us
