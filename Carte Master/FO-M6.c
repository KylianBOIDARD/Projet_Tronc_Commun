...
void FO_M6( void ){
			...
			if ( commande.Etat_Mouvement != Mouvement_non ){		//si la commande demande un mouvement
					...
					infos_Seria = transform_command_Seria(commande) ;			//on cr�e une variable de commande compr�hensible � envoyer � FO_M2
        	...
        	case Servo_H :
						duree_depl = FO_M3((int)commande.Servo_Angle+90 );	//marche pas car dur�e en float
        		infos.Etat_BUT_Servo = BUT_Servo_H ;
						break;
        	case Servo_V:
						//appel � carte slave FFS3
						//send_slave(0);
        		break;
        	default:
        		break;
			...
			if (commande.Etat_Lumiere != Lumiere_non){
					//appel � carte slave FFS1
					send_slave(1);
			
			}
			if(commande.Etat_Photo != Photo_non){
					//appel � carte slave FFS4
					//send_slave(2);
			}
...
//----------- CALCUL DE L'ANGLE DE DEPLACEMENT -----------------------------------
					alpha = (180/PI)*atan2( command.Coord_X, command.Coord_Y );	//calcul de l'angle de triangle de d�placement au format trigo en degr�
					...
					x.Vitesse_Mot2 = 160*0.05 ;			//qui sera de 5% pour tout le d�placement
					if(alpha < 0 ){		//si alpha n�gatif donc rotation � droite	
...
//----------- CALCUL DE LA NORME DE LA DISTANCE A PARCOURIR -----------------------
					...
					Coord_X_absolu += command.Coord_X ;		//mise � jour de la position 
					Coord_Y_absolu += command.Coord_Y ;		//absolue de la base dans le rep�re
					x.Etat_Commande = digo_1_2 ;
					FO_M2(x) ;		//on parcourt la distance 
					x.Etat_Commande = Pids ;
					while (FO_M2(x).Read_Pids != 0 ){} ;  //on attend la fin du dep
//----------- ORIENTATION FINALE DE LA BASE SELON L'ANGLE DESIRE -------------------- 
					alpha = commande.Angle - alpha ;	//calcul angle de d�placement pour arriver � l'orientation finale demand�e
					//Angle_relatif = commande.Angle ; 	//on enregistre l'orientation de la base pour un prochain d�placement
					if(alpha < 0 ){		//si alpha n�gatif donc rotation � droite	
							x.Ticks_mot1 = -abs( ((139*2*PI)*(float)alpha/360 )*3.312 )*0.7 ;					
							x.Ticks_mot2 = abs( ((139*2*PI)*(float)alpha/360 )*3.312 )*0.7 ;
							x.Etat_Commande = digo_1_2 ;
...
//------------------------------------------------------------------------------
//---------------- Fonction transition commande T�l�metres --------------------
//------------------------------------------------------------------------------
void command_Telemetre( void ){
		char i = 0;
		int temps_dep = 0 ;
		int mesure = 0 ;	//initialise une variable pour stocker la distance de l'obstacle	
		char angle = -90 ;  //angle de d�part pour detect par balayage
		nb_mesures = 0 ;
		for(i;i<taille_tab_mesures;i++){
			tab_mesures[i] = 0 ;		//on vide le tableau des mesures pr�c�dentes
    }
		...
		switch (commande.Etat_DCT_Obst){
			case oui_180:
				if(commande.Type_DCT_Obst == DCT_unique){	
						...
						nb_mesures += 1 ;		//on incr�mente le nb de mesures
				}
				if(commande.Type_DCT_Obst == DCT_balayage){
						while (angle < 90){
							...
							tab_mesures[nb_mesures] = mesure ;	//on stocke la detection
							nb_mesures += 1 ;		//on incr�mente le nb de mesures
							angle += commande.DCT_Obst_Resolution ;	//on incr�mente l'angle	
            }
				}
				...
				infos.Nbre_Val_obst = nb_mesures ;		//on set le nbr de mesures effectu�es
			...
			case oui_360:
				if(commande.Type_DCT_Obst == DCT_unique){	
						...
						mesure = FO_M4(2) ;		//on demande une detection arri�re
						tab_mesures[nb_mesures] = mesure ;	//on stocke la detection
						nb_mesures += 1 ;		//on incr�mente le nb de mesures
				}
				if(commande.Type_DCT_Obst == DCT_balayage){
